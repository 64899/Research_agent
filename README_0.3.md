...
本仓库当前已完成 V0.3 版本：在 V0.2 BM25 + Dense Hybrid Retrieval 基础上，新增 CrossEncoder Rerank 和规则级 Citation Check。系统能够先宽召回候选 chunks，再对候选进行精排，交给 vLLM 生成回答，展示检索来源，并检查回答正文中是否包含 citation marker。
...
- 当前最佳链路为 `Hybrid Retrieval top-10 → CrossEncoder Rerank → top-3 → vLLM answer + Sources → Citation Check`；
...
- Citation Check 能作为质量提示，发现当前本地模型没有稳定输出 citation marker 的问题；
...
这不是 Citation Check 的错误，而是说明当前本地模型虽然能生成较好的回答，但没有稳定遵守 citation marker 格式。V0.3 的目标是“能检查并提示”，不是强制模型一定在正文中生成合规引用。当前最低要求是：回答后必须展示 `Sources`，方便用户回到原文检查证据。
...
Citation Check: 能检测回答中是否缺少正文引用编号，并作为质量提示
```
...
- 当前 Qwen2.5-3B-Instruct 没有稳定输出 `[1]`、`[2]` 这类 citation marker，因此正文引用编号暂不作为硬性要求；
...
本快照对应 **V0.3 Hybrid Retrieval + Rerank + Citation Check**。此时系统已经能通过 Hybrid 进行宽召回，再用 CrossEncoder 对候选 chunks 精排，并用规则级检查发现模型回答正文是否缺少 citation marker。正文 citation marker 暂不作为硬性要求，但回答后必须展示 `Sources`。
